...
class Database {
	public function db_update($table_name,$db_columns,$db_conditions,$isOr=false)
	{
        ...
        if (mysqli_connect_errno()) {       ////throw an error if there is a connection problem
            ...
            $db_conn->close();
            as_error('DB: ' . $q . PHP_EOL . ' result:' . $result);
    ...
    public function db_create($table_name,$db_columns)
    {
        ...
        if (mysqli_connect_errno()) {       ////throw an error if there is a connection problem
            ...
            $db_conn->close();
            as_error('DB: ' . $q . PHP_EOL . ' result:' . $id);
    ...
    public function db_retrieve($table_name,$db_columns,$db_conditions='',$db_extra=null,$isOr=false)
    {
        ...
        as_error('Checking for key in cache: ' . $key);
        if(!is_null($data))
        {
            as_error('Cache found: ' . $key);
        ...
        if (mysqli_connect_errno()) {
            ...
            as_error('DB: ' . $q . PHP_EOL .  print_r($result_set,true));
                ...
                if ($result) {
                    do {
                        ...
                        if(isset($result_set[0]['row_count']))
                        {
                            ...
                            //as_error('row_count in multiquery found: row_count=' . $row_count);

                        } else
                        {
                            if(!empty($result_set)) {
                                as_error('main result set found: ' . print_r($result_set,true));
